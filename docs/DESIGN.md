...
- **Phase 5 (완료)** — 문서별 상세보기/문서형식보기(A4 커버+개정이력표+본문, 브라우저 인쇄로 PDF 출력) 및 프로젝트 단위 DHF 통합 뷰(`/projects/[id]/dhf`) 추가. DHF는 별도 데이터 모델 없이 프로젝트의 모든 `documents`+`revisionHistory`를 모아 문서 완결성(8/8) 현황과 전체 개정이력을 시간순으로 보여주는 읽기전용 집계 페이지로, 3.5절 설계대로 구현. 동일한 A4 인쇄 CSS(`globals.css`)를 재사용해 PDF 출력 가능
